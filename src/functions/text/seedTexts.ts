...
function generateRandomText(difficulty: 'easy' | 'medium' | 'hard' | 'expert', category: string): TypingText {
  const words = [
...
const seedTextsHandler = async (
  ...
  try {
    ...
    logger.info('Seeding texts with parameters', { category, difficulty, count });
    
    let textsToSeed: TypingText[] = [];
    
    // If specific category and difficulty are provided, generate texts for that combination
    if (category && difficulty) {
      const validDifficulties: ('easy' | 'medium' | 'hard' | 'expert')[] = ['easy', 'medium', 'hard', 'expert'];
      const typedDifficulty = validDifficulties.includes(difficulty as any) ? difficulty as 'easy' | 'medium' | 'hard' | 'expert' : 'medium';
      
      for (let i = 0; i < parseInt(count); i++) {
        textsToSeed.push(generateRandomText(typedDifficulty, category));
      }
    } else {
    ...
    logger.info(`Seeding ${textsToSeed.length} texts...`);
    ...
    for (const textData of textsToSeed) {
      try {
        ...
        errorCount++;
        logger.error('Error seeding text', { error: error instanceof Error ? error.message : error });
      }
    }
    ...
    logger.error('Failed to seed texts', { error });
    return internalServerError('Failed to seed texts');
  }
};

export const handler = seedTextsHandler; 